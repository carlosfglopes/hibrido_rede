#!/usr/bin/env python3
"""
Autonomous UAV agent — MissionFormation (Model 3: Hybrid FSM + UUPS
Proxy). Runs independently on each RPi. Connects to the local ERC1967
proxy via web3.py and acts with its own key (msg.sender) — same as
Model 1 and Model 2 (registration by the authority, self-service
position/violation actions), but mission progress is read through the
missionState enum (real FSM, unlike Model 2 which uses boolean flags).

Usage:
    UAV_ID=UAV1 PRIVATE_KEY=0x... CONTRACT_ADDRESS=0x... \
    BASE_X=0 BASE_Y=0 python3 agent_missionformation.py
...
Environment variables:
    ...
    POSITION_INTERVAL (seconds between updatePosition, default: 8)
...
# States
MISSION_STATES = {0: "SETUP", 1: "ACTIVE", 2: "RECONFIGURING_FORMATION",
    ...
    while True:
        try:
            ...
            if state in TERMINAL_STATES:
                log(f"Mission {MISSION_STATES.get(state, state)}. Agent terminating.")
                break
            ...
            for i in range(count):
                ...
                peer_state, peer_x, peer_y, dist_centroid_sq = status[0], status[1], status[2], status[5]
            ...
            log(f"mission={MISSION_STATES.get(state, state)} myState={UAV_STATES.get(my_state, my_state)}")
