// scripts/authority_missionfail.js
// Authority — MissionFail (Model 3: Hybrid FSM + UUPS Proxy). Runs on the
// PC. Registers UAVs, creates tasks, starts the mission, simulates a
// malicious-behavior incident on UAV4, waits for agent votes, finalizes
// the incident and reconfigures. If the mission survives without further
// failures, it closes it with completeMission() (a new function, added
// because a success state was missing — same fix as Model 1).
//
// Usage:
//   npx hardhat run scripts/authority_missionfail.js --network rede-hibrido
...
const UAV_ADDRESSES = [
  "0xf39Fd6e51aad88F6F4ce6aB8827279cffFb92266", // UAV1 — rpi1
  "0x70997970C51812dc3A010C7d01b50e0d17dc79C8", // UAV2 — rpi2
  "0x3C44CdDdB6a900fa2b585dd299e03d12FA4293BC", // UAV3 — rpi3
  "0x90F79bf6EB2c4f870365E785982E1f101E93b906", // UAV4 — rpi4
];
...
async function main() {
  ...
  if (!fs.existsSync(addrPath)) throw new Error("fail_addresses.json not found — run deploy_missionfail.js first.");
  ...
  sep("AUTHORITY — MissionFail (Model 3)");
  ...
  while (true) {
    ...
    log(`State: ${MISSION_STATES[state]} | Votes — Failed:${s.vFailed} Byzantine:${s.vByzantine} Reject:${s.vReject}`);
  ...
  const s7 = await proxy.getMissionSummary();
  if (Number(s7.state) === 3) {
  ...
  for (let i = 0; i < 10; i++) {
    const s = await proxy.getMissionSummary();
    const state = Number(s.state);
    log(`State: ${MISSION_STATES[state]} | Failures: ${s.failures} | Active UAVs: ${s.activeUAVs} | Tasks: ${s.activeTasks}`);
    if (state === 6) {
      log("Mission ABORTED.");
      finished = true;
      break;
  ...
  if (!finished) {
    ...
    if ([1, 4, 5].includes(state)) {
      ...
      log(`Mission ended in unexpected state: ${MISSION_STATES[state]}`);
    }
  }
  ...
  await finishAndSaveMetrics({
    ...
    fromBlock, model: "Modelo3-Hibrido", scenario: "Fail", log,
  });
