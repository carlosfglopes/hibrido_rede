...
/// @notice Model 3 (hybrid) counterpart: same formation-tolerance/consensus
///         behavior, upgraded via UUPS instead of an FSM.
...
contract MissionFormationV1 is Initializable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    function finalizeFormationChange()
        external onlyOwner inMissionState(MissionState.RECONFIGURING_FORMATION)
    {
    ...
    function updatePosition(int256 _x, int256 _y)
        external virtual onlyRegisteredActiveUAV inOperationalState
    {
        ...
        )
    {
        ...
        return (
            u.state, u.x, u.y, u.lastUpdate, u.violationCount,
            _squaredDist(u.x, u.y, centroidX, centroidY),
            violationVotes[_uav], recoveryVotes[_uav]
        );
    }
        ...
        )
    {
        ...
        uint256 timeLeft   = (transition && block.timestamp < transitionEnd)
            ? transitionEnd - block.timestamp : 0;
    ...
    function getSwarmCounts()
        external view
        returns (uint256 okCount, uint256 lateCount, uint256 outOfFormationCount, uint256 inactiveCount)
    {
        for (uint256 i = 0; i < uavList.length; i++) {
            UAVState s = uavs[uavList[i]].state;
    ...
    function _checkSwarmHealth() internal virtual {
